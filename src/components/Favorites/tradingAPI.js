...
export default class TradingViewWidget extends PureComponent {
  ...
  initWidget = () => {
    if(window.TradingView){
      // console.log(this.props.id);
        ...
        const config = { ...widgetConfig, container_id: this.props.id || CONTAINER_ID };
    
        if (config.autosize) {
          delete config.width;
          delete config.height;
          const container = document.getElementById(this.props.id || CONTAINER_ID);
  ...
  cleanWidget = () => {
    document.getElementById(this.props.id || CONTAINER_ID).innerHTML = '';
  };

  render = () => <article id={this.props.id || CONTAINER_ID} />;
}